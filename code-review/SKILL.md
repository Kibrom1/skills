...
name: code-review
description: >
  Perform a thorough, structured code review on any file, directory, diff, or PR in this workspace.
  Use this skill whenever the user asks to "review", "check", "audit", or "look over" code — including
  phrases like "can you review this PR", "review my changes", "does this code look good", "what do you
...
- **What**: a file path, directory, git diff, PR description, or pasted code
...
- **GitHub PR?**: if the user mentions a PR number and repo (e.g. "review PR #5 in myrepo"), note the
  owner, repo, and PR number — you'll post the review to GitHub automatically after completing it (see Step 5)
...
**For GitHub PRs:** use `mcp__github__get_pull_request` to fetch PR metadata and
`mcp__github__get_pull_request_files` to get the diff. The `patch` field on each file contains the
...
| Mixed (e.g., FastAPI + React in same repo) | Multi-stack | Read both relevant files |
...
## Step 5: Post the review to GitHub (for GitHub PRs only)

After writing the review, always post it to the GitHub PR using
`mcp__github__create_pull_request_review`. Do this automatically — don't ask the user, just post it.
...
| Blockers or should-fix items, PR author is **not** the authenticated user | `REQUEST_CHANGES` |
| Blockers or should-fix items, PR author **is** the authenticated user (own PR) | `COMMENT` |

GitHub does not allow a user to request changes on their own PR (returns a 422 error). If unsure
whether it's the user's own PR, try `REQUEST_CHANGES` first and fall back to `COMMENT` on a 422.
...
mcp__github__create_pull_request_review(
  ...
  repo        = <repo name>,
  pull_number = <PR number as integer>,
  event       = "REQUEST_CHANGES" | "APPROVE" | "COMMENT",
  body        = <full review text from Step 4>
)
```

After a successful post, share the PR link with the user:
